...
그래서 챗 쥐피티 도움도 받고 의심가는 것들 좀 뒤져보고 있었는데,
...
기존에 이 서비스를 담당했던 분들은 서버 개발자가 아니였고, 지금 쓰는 언어도 쓰시던 분들이 아니었고, 기능의 구현도 현재와는 달랐다.
...
그래서 이런저런 상황에서 아래처럼 변경을 했다.
- cpu 중심 기능만을 위한 별도의 프로세스만 유지
- io 중심 기능은 모두 메인 쓰레드에서 일어나도록 하되, io 성능은 유지될 수 있도록 쓰레드풀 숫자 튜닝
- 메인 쓰레드, 별도 프로세스 이렇게 둘을 위해 컨테이너 하나 당 2vcpu 할당
